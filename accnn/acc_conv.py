...
def conv_vh_decomposition(model, args):
  ...
  if (args.layer + '_bias') in model.arg_params:
  ...
  def sym_handle(data, node):
    ...
    pad = eval(node['attrs']['pad'])
    name = node['name']
    no_bias = True if node['attrs']['no_bias'] == 'True' else False
    ...
    sym1 = mx.symbol.Convolution(data=data, kernel=kernel1, pad=pad1, num_filter=num_filter, no_bias=no_bias, name=name1)

    name2 = name + '_h'
    kernel2 = tuple((1, kernel[1]))
    pad2 = tuple((0, pad[1]))
    num_filter = W2.shape[0]
    sym2 = mx.symbol.Convolution(data=sym1, kernel=kernel2, pad=pad2, num_filter=num_filter, no_bias=no_bias, name=name2)
    return sym2
